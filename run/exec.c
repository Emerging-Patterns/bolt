// bootrun.exec: runs a program with its arguments and answers the exit status
// on its own first line, then everything the program printed.
//
// The arguments arrive in one string, newline separated, and are handed to
// execvp as a vector, so no shell sees them. The child gets /dev/null for
// stdin and one pipe for stdout and stderr, so it cannot touch this program's
// own stdio.
#include <fcntl.h>
#include <sys/wait.h>
#include <unistd.h>

Term bootrun_exec_run(Env e, Term* f, IoWork* w) {
  uint64_t n = 0;
...
static void __attribute__((constructor)) bootrun_exec_use(void) {
  io_eff(CID_BOOTRUN_EXEC, bootrun_exec_run, 0);
}
