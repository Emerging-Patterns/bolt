...
nothing under `bolt/` imports this. It exists for the stay-list
host/integration tests in the repo's top-level `tests/`, which have to
drive the real `bend`, the real `node` and the real `nix`, because those
are the only things that can answer what those tests ask: that bolt still
builds with bare `bend`, that the server survives being spawned by node
over sockets, and that the packaged build and its C toolchain still work.
