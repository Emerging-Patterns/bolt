...
| BOLT-SYN-3 | The tree drops no token: its leaves are the significant tokens in order, except that a run of `>` closing angle groups becomes one `>` leaf per character at consecutive columns. | Proved | proved | syntax/LAWS.bend leaves |
