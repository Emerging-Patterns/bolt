...
| BOLT-RULE-S003 | `wrap` reports exactly one finding for each def header that breaks its shape and none otherwise: a one-line header wider than 120 columns through its last `:` (the whole header when it has none), counted as `space` counts a line, or a header across lines whose parameter list (opened by the first `(` with no bracket open, split by the commas at its own depth) holds a parameter that spans lines, starts on the `(` line, or starts on the line a later parameter starts on, or closes with its `)` not on a line below the last parameter's last line. | Proved | proved | bolt/rules/LAWS.bend wrap_width; bolt/rules/LAWS.bend wrap_shape_counts; bolt/rules/LAWS.bend wrap_counts |
