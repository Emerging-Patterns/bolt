...
| BOLT-SCOPE-4 | Path exemptions are decided by the path alone, never by content: two files at one path are exempt from `law` alike whatever each holds, and on a path a per-file rule's header exempts (a LAWS.bend, a PROOF.bend, a file under a `tests` directory, and for `closed` any file but a LAWS.bend) that rule's findings are the same for every source. What a rule reads from content to skip a def, a line or a file (a def returning a proof, a file that defines `Map.put`, expectation trailers, `# lanes: native`, `main` and dotted names) is that rule's behavior, not a path exemption. | Proved | proved | bolt/rules/LAWS.bend scope_digest_exempt; bolt/rules/LAWS.bend scope_hole; bolt/rules/LAWS.bend scope_doc; bolt/rules/LAWS.bend scope_param; bolt/rules/LAWS.bend scope_law_file; bolt/rules/LAWS.bend scope_closed |
