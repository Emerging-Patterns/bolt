...
| BOLT-RULE-U004 | `concat` reports exactly a self-call argument that appends onto a parameter. | Proved | pending |  |
...
| BOLT-RULE-U006 | `fuel` reports exactly a Nat literal passed where a def of the file takes its fuel. | Proved | pending |  |
...
| BOLT-TRUST-2 | bolt's lexer, tree and outline read a file as the program bend reads, for the constructs the rules depend on. | bolt cannot call bend's parser from Bend. Four divergences are known (multi-line strings, indented continuation lines, `>` inside parentheses in `<..>`, ASCII-only identifiers); each is either fixed or narrows a rule's requirement. Indented continuation lines are a decided difference: a line indented deeper, outside brackets, is a child statement in bolt's tree (`"a"` then `    ++ "b"` is `["a" {[++ "b"]}]`), where bend reads one expression. The rules that read statements from the tree could be affected: `tail` (the self-call is the whole statement), `unused` and `shadow` (a let's scope), `table` (a let reaches the statements after it), `rewalk` (one straight piece) and `wrap` (a header's lines). Run on a continued `++`, `<>` or boolean or, a continued let value, a lambda body and a call argument, `tail`, `strict`, `concat`, `unused` and `param` report what they report on the same code on one line; `wrap` reports a header whose `-> T:` sits on the next line, which its requirement makes a finding. |
