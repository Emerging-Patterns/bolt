...
| BOLT-TRUST-3 | The directory listing effect (`bolt/walk/dir.c`, `dir.js`) returns a directory's entries, marking directories with `/`, the directory probe (`bolt/walk/disk.bend` `is_dir`) says a path is a directory exactly when it is one, and the file read effect returns a file's text. | The walk and the reads are foreign code; the planner takes their answers as given. |
