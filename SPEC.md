...
| BOLT-RULE-INERT | For every rule whose pattern is code (all but `escape`, `strings`, `chars`, `space` and `twice`), changing the contents of a comment or string literal does not change the findings. | Proved | pending | bolt/rules/LAWS.bend inert_hole; bolt/rules/LAWS.bend inert_put; bolt/rules/LAWS.bend inert_tail; bolt/rules/LAWS.bend inert_pick; bolt/rules/LAWS.bend inert_strict |
