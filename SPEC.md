...
| BOLT-RULE-U002 | `strict` reports exactly a self-call inside `Bool.and` or `Bool.or`, or on either side of `&&` or `\|\|`. | Proved | pending |  |
...
| BOLT-TRUST-2 | bolt's lexer, tree and outline read a file as the program bend reads, for the constructs the rules depend on. | bolt cannot call bend's parser from Bend. Names are ASCII in both: bend 2.0.25 rejects a non-ASCII letter in a def name, a parameter or a let binder (`def café() -> U32:` is "expected: '('"), so the lexer's ASCII-only names are no divergence. Four divergences are known (multi-line strings, indented continuation lines, `>` inside parentheses in `<..>`, a char literal holding a raw newline); each is either fixed or narrows a rule's requirement. |
