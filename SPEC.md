...
| BOLT-TRUST-2 | bolt's lexer, tree and outline read a file as the program bend reads, for the constructs the rules depend on. | bolt cannot call bend's parser from Bend. A `>` inside `(..)`, `[..]` or `{..}` within `<..>` is an operator, as in bend, and does not close the angle group. Four divergences are known (multi-line strings, indented continuation lines, ASCII-only identifiers, a char literal holding a raw newline); each is either fixed or narrows a rule's requirement. |
